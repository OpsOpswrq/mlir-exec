...
void Exec4(llvm::StringRef name, int64_t lhsValue, int64_t rhsValue, unsigned width = 16, int64_t cnt = 1){
    // llvm::outs() << "\n\n";
    // Initialize MLIR components
    mlir::MLIRContext context;
    ...
    // Get the dialect and print welcome message
    auto *dialect = context.getLoadedDialect<mlir::exec::ExecDialect>();
    // dialect->sayHello(name);
    
    // Set up builder and location
    mlir::OpBuilder builder(&context);
    auto loc = builder.getUnknownLoc();
    ...
    // Create function type with two parameters and one return value
    auto funcType = builder.getFunctionType({lhsType, rhsType}, {lhsType});
    auto func = builder.create<mlir::func::FuncOp>(loc, "test_add", funcType);
    ...
    // Get block arguments as operands (these are function parameters, not constants)
    auto lhsVal = entryBlock.getArgument(0);
    auto rhsVal = entryBlock.getArgument(1);
    
    // Create cnt attribute
    auto cntAttr = mlir::exec::ExecCIntAttr::get(&context, cnt);
    
    // Create AddOp operation
    auto addOp = builder.create<mlir::exec::AddOp>(
        loc,
        lhsVal,
        rhsVal,
    ...
    // Create return operation, returning the result of addOp
    builder.create<mlir::func::ReturnOp>(loc, addOp.getResult());
    
    // Print the module before applying the pass
    ...
    optPM.addPass(mlir::exec::createExecPasses());
    
    // llvm::outs() << "Running ExecTestPass...\n";
    if (pm.run(module).failed()) {
        llvm::errs() << "Pass execution failed\n";
        return;
    }
    
    // Print the result after applying the pass
    // llvm::outs() << "Module after pass application:\n";
    // module->print(llvm::outs());
}

void Exec5(llvm::StringRef name, int64_t lhsValue, int64_t rhsValue, unsigned width = 16, int64_t cnt = 1){
    llvm::outs() << "\n\n";
    // Initialize MLIR components
    mlir::MLIRContext context;
...
int main(int argc, char **argv) {
    if (argc == 2) {
        ...
        Exec5(argv[1], lhs, rhs, width, cnt);
    } else {
        std::cerr << "Usage:\n"
                  << "  " << argv[0] << " <name>\n"
                  << "  " << argv[0] << " <name> <int>\n"
